use crate::{
    ...
    error::DhtError,
    iterative_lookup::{start_bootstrap, start_refresh, start_search},
    message::{
...
impl Dht {
    ...
    pub fn shutdown(self) -> Result<(), DhtError> {
    ...
    fn refresh_table(&self) {
        ...
        for idx in buckets_to_refresh {
            ...
            if !k_closest_nodes.is_empty() {
                start_refresh(target, our_node_id, dht_node_tx, af, &k_closest_nodes);
            }
        }
    }
    ...
    async fn handle_command(&mut self, cmd: DhtNodeCommand) {
        match cmd {
            ...
            DhtNodeCommand::SearchTask {
                ...
                let k_closest_nodes = self.routing_table.read().unwrap().find_closest(&target, 8);
                let jh = start_search(target, our_node_id, dht_node_tx, port, af, &k_closest_nodes);
                tokio::spawn(async move {
            ...
            DhtNodeCommand::Bootstrap { addrs, reply } => {
                ...
                let jh = start_bootstrap(addrs, node_id, tx, family);
                tokio::spawn(async move {
                    let _ = jh.await;
                    let _ = reply.send(());
                });

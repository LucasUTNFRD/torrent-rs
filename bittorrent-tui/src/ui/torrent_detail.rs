...
pub fn draw_torrent_detail(f: &mut Frame, app: &mut App, area: Rect) {
    let details_block = Block::default()
        .borders(Borders::ALL)
        .title(" Details ")
        .border_style(if app.active_tab == Tab::Details && app.mode == Mode::Main {
    ...
    if let Some(details) = &app.selected_torrent {
        ...
        match app.details_tab {
            0 => {
                let rows: Vec<Row> = details.peers.iter().map(|p| {
                    Row::new(vec![
                ...
                .header(Row::new(vec!["IP", "Down", "Up", "Client"]).style(Style::default().add_modifier(Modifier::BOLD)))
                .block(Block::default());
                f.render_widget(table, details_chunks[1]);
            }
            1 => {
                let rows: Vec<Row> = details.trackers.iter().map(|t| {
                    Row::new(vec![
                ...
                .header(Row::new(vec!["URL", "Status"]).style(Style::default().add_modifier(Modifier::BOLD)))
                .block(Block::default());
                f.render_widget(table, details_chunks[1]);
            }
            2 => {
                let rows: Vec<Row> = details.files.iter().map(|fi| {
                    Row::new(vec![
                ...
                .header(Row::new(vec!["Path", "Size", "Progress"]).style(Style::default().add_modifier(Modifier::BOLD)))
                .block(Block::default());
                f.render_widget(table, details_chunks[1]);
            }
            _ => unreachable!(),

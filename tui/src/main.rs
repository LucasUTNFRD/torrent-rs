...
async fn handle_key_event(key: KeyEvent, app: &mut App, dialog: &mut DialogState) -> Result<()> {
    match dialog {
...
fn handle_detail_key(key: KeyEvent, app: &mut App) {
    let View::Detail { tab, .. } = app.view else {
        return;
    };
    let mut new_tab = tab;
                ...
                let i = match state.selected() {
                    Some(i) => {
                        if app.detail.files.is_empty() {
                            ...
                            (i + 1).min(app.detail.files.len() - 1)
                        }
                    }
                    None => 0,
                };
                ...
                let i = match state.selected() {
                    Some(i) => {
                        if app.detail.peers.is_empty() {
                            ...
                            (i + 1).min(app.detail.peers.len() - 1)
                        }
                    }
                    None => 0,
                };
...
async fn handle_session_event(event: SessionEvent, app: &mut App) {
    match event {
...
fn render_header(f: &mut Frame, area: Rect, _app: &App) {
    let title = Paragraph::new("Torrent-RS TUI")
...
fn render_status_bar(f: &mut Frame, area: Rect, app: &App) {
    let (downloading, seeding, paused) =
        app.progress
            .values()
            .fold((0u32, 0u32, 0u32), |(dl, sd, pa), p| match p.state {
...
fn render_overview_details(f: &mut Frame, area: Rect, app: &App) {
    ...
    let rows: Vec<Line> = vec![
        ...
        kv_line(
            "Size",
            meta.map(|m| fmt_size(m.total_bytes))
                .as_deref()
                .unwrap_or("…"),
            label_style,
        ),
    ...
    let rows: Vec<Row> = app
        ...
        .map(|p| {
            ...
            let flags = format!(
                ...
                if p.info.extension_flags.extension_protocol {
                    "H"
                } else {
                    " "
                },
                if p.info.extension_flags.metadata {
                    "E"
                } else {
                    " "
                },
...
fn render_trackers(f: &mut Frame, area: Rect, app: &App) {
    ...
    let table = Table::new(rows, widths)
        .header(
            Row::new(["URL", "Status", "Peers", "Last Error"])
                .style(Style::default().add_modifier(Modifier::BOLD))
                .bottom_margin(1),
        )
